    ...
    getRacine(id) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const sqlQuery = `
                    ...
                    select nomUtilisateur as nomFichier, nom as VraiNom, tokenId as tokenId, 'fichier' AS Type, id from dossier where type like 'Fichier' AND dossierId IS NULL AND userId = ?
                    UNION ALL
            ...
            /*const sqlQuery = `SELECT T.blobName as nomFichier, T.id, 'fichier' AS Type FROM token T LEFT JOIN dossier
                D ON T.id = D.id WHERE D.tokenId IS NULL AND T.userId = 2 and T.blobName is not NULL
